...
/**
 * The one role name treated as a protected identity, not just a row a
 * Roles-admin can rename/delete like any other: orycms/auth/auth.ts seeds
 * it as the bootstrap owner's role, and orycms/projects/projects.repo.ts
 ...
 * string.
 */
export const ORYCMS_SUPER_ADMIN_ROLE = "Super Admin";
 ...
 * contains `*:manage` for every resource, so this is never a restriction
 * for them in practice.
 */
export function permissionsWithinCeiling(
  granted: Set<string>,
  requested: { resource: string; action: string }[],
): boolean {
  return requested.every(
    ({ resource, action }) => granted.has(`${resource}:${action}`) || granted.has(`${resource}:manage`),
  );
}
