...
describe("syncOryCMSDefaultRoles", () => {
  it("upserts all 5 default roles", async () => {
    ...
    expect(roles).toEqual(["Super Admin", "Admin", "Editor", "Author", "Viewer"]);
  });
});
...
describe("hasOryCMSPermission", () => {
  ...
  it("Super Admin has full access via 'manage' on every resource", async () => {
    const pool = poolFor("Super Admin");
    expect(await hasOryCMSPermission("Super Admin", "settings", "delete", pool)).toBe(true);
    expect(await hasOryCMSPermission("Super Admin", "roles", "manage", pool)).toBe(true);
    expect(await hasOryCMSPermission("Super Admin", "migrations", "create", pool)).toBe(true);
  });
  ...
  it("manage permission implies all actions on same resource", async () => {
    ...
    expect(await hasOryCMSPermission("Super Admin", "content", "publish", pool)).toBe(true);
    expect(await hasOryCMSPermission("Super Admin", "content", "delete", pool)).toBe(true);
  });
});
...
describe("requireOryCMSPermission", () => {
  ...
  it("resolves when permission is granted", async () => {
    const pool = makePool(() => ({ rows: [{ resource: "content", action: "manage" }] }));
    await expect(
      requireOryCMSPermission({ roleName: "Super Admin" }, "content", "publish", pool),
