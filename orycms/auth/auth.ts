...
// Constant-time-ish: always bcrypt.compare against SOME hash, even when the
// real one is absent, so a missing/short-circuited comparison can't leak
// "does this row exist" via timing. Shared by authenticateOryCMSUser (keyed
// by email, possibly no row) and verifyOryCMSUserPassword (keyed by a
// userId we already know exists, but kept on the same safe primitive).
const DUMMY_PASSWORD_HASH = "$2a$12$invalidhashfortimingprotection0000000000000000000000";

async function compareWithDummyHash(password: string, hash: string | null | undefined): Promise<boolean> {
...
/**
 * Creates the Super Admin role (if absent) and the first admin user.
...
export async function createOryCMSInitialOwner(
  ...
  const userResult = await pool.query<OryCMSAuthUser>(
    `INSERT INTO orycms_users (id, name, email, "passwordHash", status, "roleId")
     VALUES (gen_random_uuid(), $1, $2, $3, 'active', $4)
     RETURNING id, name, email, "roleId", status, "mfaEnabled"`,
    [input.name.trim(), input.email.toLowerCase().trim(), passwordHash, roleId],
  );
...
export async function authenticateOryCMSUser(
  ...
  const result = await pool.query<OryCMSAuthUser & { passwordHash: string }>(
    `SELECT id, name, email, "passwordHash", status, "roleId", "mfaEnabled"
     FROM orycms_users
     WHERE email = $1
     LIMIT 1`,
    [normEmail],
  ...
  const valid = await compareWithDummyHash(password, user?.passwordHash);
 ...
 *
 * Returns a plain boolean rather than throwing, so callers can fold this
 * into a larger fail-closed check (e.g. "password AND TOTP both valid")
 * without needing to catch a specific error type first.
 */
export async function verifyOryCMSUserPassword(
  pool: Pool,
  userId: string,
  password: string,
): Promise<boolean> {
  const result = await pool.query<{ passwordHash: string; status: string }>(
    `SELECT "passwordHash", status FROM orycms_users WHERE id = $1 LIMIT 1`,
    [userId],
  );
  ...
  return Boolean(user) && valid && user?.status === "active";
}
...
export async function getOryCMSCurrentSession(
  ...
  const result = await pool.query<OryCMSSessionData>(
    `SELECT u.id AS "userId", u.name, u.email, r.name AS "roleName", u."mfaEnabled"
