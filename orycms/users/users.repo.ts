...
/** Only @orynticlabs.com addresses may be granted OryCMS admin access. */
export const ORYCMS_ALLOWED_EMAIL_DOMAIN = "orynticlabs.com";

export function isAllowedOryCMSEmailDomain(email: string): boolean {
  return email.toLowerCase().trim().endsWith(`@${ORYCMS_ALLOWED_EMAIL_DOMAIN}`);
}
...
export async function listOryCMSUsers(pool: Pool = getOryCMSPool()): Promise<OryCMSUserRecord[]> {
  const result = await pool.query<OryCMSUserRecord>(
    `SELECT u.id, u.name, u.email, u.status, u."roleId", r.name AS "roleName"
     FROM orycms_users u
...
export async function createOryCMSUser(
  ...
  if (!isAllowedOryCMSEmailDomain(input.email)) {
    throw Object.assign(
      new Error(`Only @${ORYCMS_ALLOWED_EMAIL_DOMAIN} email addresses can be added.`),
  ...
  const result = await pool.query<OryCMSUserRecord>(
    `INSERT INTO orycms_users (id, name, email, "passwordHash", status, "roleId")
     ...
     RETURNING id, name, email, status, "roleId"`,
    [
 ...
 */
export async function countOryCMSActiveUsersByRoleName(
  roleName: string,
  pool: Pool = getOryCMSPool(),
): Promise<number> {
