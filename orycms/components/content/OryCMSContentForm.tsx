  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-border pt-5">
        {hasDraft ? (
          <>
            <Can resource="content" action={isEdit ? "update" : "create"}>
              <Button onClick={() => handleSave(true)} disabled={saving} variant="outline">
                {saving && <OryCMSSpinner className="h-3.5 w-3.5" />}
                Save as draft
              </Button>
            </Can>
            {isEdit && !isPublished && (
              <Can resource="content" action="publish">
                <Button onClick={handlePublish} disabled={publishing}>
                  {publishing && <OryCMSSpinner className="h-3.5 w-3.5" />}
                  Publish
                </Button>
              </Can>
            )}
            {isEdit && isPublished && (
              <Can resource="content" action="publish">
                <Button onClick={handleUnpublish} disabled={unpublishing} variant="outline">
                  {unpublishing && <OryCMSSpinner className="h-3.5 w-3.5" />}
                  Unpublish
                </Button>
              </Can>
            )}
