...
import { IMAGES, TECH_GROUPS, PROJECTS } from "@site/data/content";
import { WorldMap } from "@site/components/ui/world-map";
import { SectionHead } from "@site/components/site/Reveal";

const TECH_SAMPLE = TECH_GROUPS.flatMap((g) => g.tools).filter((_, i) => i % 7 === 0).slice(0, 10);
...
const FEATURED_PROJECT = PROJECTS[0];
...
function SkeletonTechStack() {
  const rowA = TECH_SAMPLE.slice(0, 5);
  const rowB = TECH_SAMPLE.slice(5, 10);
  const chip = (name, idx) => (
    <motion.div
      key={name + idx}
      whileHover={{ scale: 1.08, zIndex: 10 }}
      style={{ rotate: ((idx * 37) % 20) - 10 }}
      className="-mr-3 mt-4 shrink-0 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-semibold text-white/80 shadow-lg backdrop-blur-sm"
    >
      {name}
    </motion.div>
  );
  ...
  return (
    <div className="relative flex h-full flex-col items-start gap-6 overflow-hidden p-6">
      <div className="-ml-16 flex flex-row">{rowA.map(chip)}</div>
      <div className="flex flex-row">{rowB.map(chip)}</div>
      ...
      <div className="mx-auto max-w-7xl">
        <div className="px-8">
          <SectionHead
            ...
            title="Built to do the whole job, not just part of it."
