            ...
            {INDUSTRIES.map((ind, i) => (
              <Reveal key={ind.name} delay={0.04 * i}>
                <div
                  ...
                  className="group flex items-start justify-between gap-6 border-b border-white/10 py-8 transition-colors duration-300 hover:border-brand-orange/50"
                >
                  <div className="flex gap-6">
                    <span className="pt-1 font-display text-sm font-bold text-white/30">{String(i + 1).padStart(2, "0")}</span>
                    <div>
                      ...
                      <p className="mt-2 max-w-lg text-sm leading-relaxed text-white/50">{ind.build}</p>
                    </div>
                  </div>
                  <ArrowUpRight className="mt-2 h-5 w-5 shrink-0 text-white/20 transition-all duration-300 group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-brand-orange" />
                </div>
              </Reveal>
            ))}
          </div>
