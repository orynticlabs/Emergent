...
import { createOryCMSUser, isAllowedOryCMSEmailDomain, ORYCMS_ALLOWED_EMAIL_DOMAIN } from "@/users";
  ...
  try {
    ...
    if (!body.name?.trim()) {
      return toErrorResponse(
        Object.assign(new Error("Name is required."), { code: "VALIDATION_ERROR", statusCode: 422 }),
      );
    }
    if (!body.email) {
      return toErrorResponse(
    ...
    if (!isAllowedOryCMSEmailDomain(email)) {
      return toErrorResponse(
        Object.assign(
          new Error(`Only @${ORYCMS_ALLOWED_EMAIL_DOMAIN} email addresses can be invited.`),
    ...
    if (body.roleId) {
      ...
      if (targetRole.name === ORYCMS_SUPER_ADMIN_ROLE && session.roleName !== ORYCMS_SUPER_ADMIN_ROLE) {
        return toErrorResponse(
          Object.assign(new Error("Only a Super Admin can invite someone as Super Admin."), {
            code: "FORBIDDEN",
            statusCode: 403,
          }),
        );
