...
import { NAV_LINKS, SERVICES, PRODUCTS, INDUSTRIES } from "@/data/content";
              ...
              <div data-testid={`sitemap-group-${group.title.toLowerCase()}`}>
                ...
                <ul className="mt-5 space-y-3">
                  {group.links.map((link) => (
                    <li key={link.label}>
                      <Link to={link.url} className="group inline-flex items-center gap-2 text-sm text-black/60 transition-colors duration-300 hover:text-brand-orange">
                        {link.label}
